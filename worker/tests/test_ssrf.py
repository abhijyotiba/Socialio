...
def test_private_10_blocked():
    with patch("socket.gethostbyname", return_value="10.0.0.1"):
        assert _is_ssrf_safe("http://internal.corp/") is False
...
def test_link_local_aws_metadata_blocked():
...
def test_private_172_16_blocked():
    with patch("socket.gethostbyname", return_value="172.16.0.1"):
        assert _is_ssrf_safe("http://172.16.0.1/") is False


def test_dns_failure_returns_false():
    import socket
    with patch("socket.gethostbyname", side_effect=socket.gaierror("nxdomain")):
        assert _is_ssrf_safe("http://does-not-exist.invalid/") is False
